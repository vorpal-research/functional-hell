...
struct Nth {
	...
	static inline auto apply(Head&& head, Args&&... args) -> decltype(Nth<N-1>::apply(std::forward<Args>(args)...)) {
...
struct Nth<0U> {
	template<class Head, class ...Args>
	static inline auto apply(Head&& head, Args&&...) -> decltype(std::forward<Head>(head)) {
...
public:
	template<class ...Args>
	inline auto operator()(Args&&... args) const -> decltype(Nth<N>::apply(std::forward<Args>(args)...)) {
